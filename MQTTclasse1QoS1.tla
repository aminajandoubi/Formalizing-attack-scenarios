                 ...
                 /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                          /\ LET packet2 == Head(network[_attaque_data[self].BrokerID]) IN
                             \/ /\ (packet2.type = "PUBLISH")
                                   /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "PUBLISH", sender |-> packet2.sender, topic |-> "A", QoS |-> 1]) IN
                                      /\ network' = _network
                            ...
                            \/ /\ ~((packet3.type = "PUBACK"))
                                  /\ LET _network == [network EXCEPT ![3] = Tail(network[3])] IN
                                     ...
                                     \/ /\ ~((_attaque_data[self].INJ2 = TRUE))
                                              ...
                                              /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Publisher_data, _stack >>
_attaque(self) == 
                      Lbl_7(self) \/ CONNACK(self) \/ PUBLISH(self) \/ PUBACK(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
PubStart(self) == 
                  /\ _pc[self] = "PubStart"
                  /\ cp = any
                              ...
                              \/ /\ ~((packet.type = "PUBACK"))
                                    /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                                       ...
                                       \/ /\ ~((_Publisher_data[self].PUBSUCC = TRUE))
                                                ...
                                                /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Publisher_data, _stack >>
_Publisher(self) == 
                        PubStart(self) \/ waitCONN(self) \/ tryPUB1(self) \/ waitPUB1(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
