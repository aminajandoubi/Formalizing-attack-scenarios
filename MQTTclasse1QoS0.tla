                 ...
                 /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                          /\ LET packet2 == Head(network[_attaque_data[self].BrokerID]) IN
                             \/ /\ (packet2.type = "PUBLISH")
                                   /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "PUBLISH", sender |-> packet2.sender, topic |-> "A", QoS |-> 0]) IN
                                      LET __attaque_data == [_attaque_data EXCEPT ![self].INJ2 = TRUE] IN
                                      /\ network' = _network
                                      /\ _attaque_data' = __attaque_data
                                      /\ _pc' = [_pc EXCEPT ![self] = "LBL1"]
                                      /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _Publisher_data, _stack >>
                             \/ /\ ~((packet2.type = "PUBLISH"))
                                      /\ _pc' = [_pc EXCEPT ![self] = "LBL1"]
                                      /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Publisher_data, _stack >>
LBL1(self) == 
              ...
              /\ \/ /\ (_attaque_data[self].INJ2 = TRUE)
                 ...
                 \/ /\ ~((_attaque_data[self].INJ2 = TRUE))
                          ...
                          /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Publisher_data, _stack >>
_attaque(self) == 
                      Lbl_7(self) \/ CONNACK(self) \/ PUBLISH(self) \/ LBL1(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
PubStart(self) == 
                  /\ _pc[self] = "PubStart"
                  /\ cp = any
...
LBL2(self) == 
              ...
              /\ \/ /\ (_Publisher_data[self].PUBSUCC = TRUE)
                 ...
                 \/ /\ ~((_Publisher_data[self].PUBSUCC = TRUE))
                          ...
                          /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Publisher_data, _stack >>
_Publisher(self) == 
                        PubStart(self) \/ waitCONN(self) \/ tryPUBLISH(self) \/ LBL2(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
...
Temp0 == (~ \A D \in Publisher, I \in attaque : (<> (((_Publisher_data[D].PUBSUCC = TRUE) /\ (_attaque_data[I].INJ1 = TRUE)) => (_attaque_data[I].INJ2 = TRUE))))
