                    ...
                    /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                             /\ LET packet2 == Head(network[_attaque_data[self].BrokerID]) IN
                                \/ /\ (packet2.type = "SUBSCRIBE")
                                      /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "SUBSCRIBE", sender |-> packet2.sender, topic |-> "A"]) IN
                                         /\ network' = _network
                                         ...
                                         \/ /\ ~((packet.type = "SUBACK"))
                                               /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                                                  ...
                                                  \/ /\ ~((_Subscriber_data[self].SUBSUCC = TRUE))
                                                           ...
                                                           /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
_Subscriber(self) == 
                         Sub_start(self) \/ Sub_waitCONN(self) \/ Sub(self) \/ Subwaitsubscription(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
...
Temp0 == (~ \A D \in Subscriber, I \in attaque : (<> (((_Subscriber_data[D].SUBSUCC = TRUE) /\ (_attaque_data[I].INJ1 = TRUE)) => (_attaque_data[I].INJ2 = TRUE))))
