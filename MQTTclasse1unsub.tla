------------------------------ MODULE MQTTclasse1unsub ---------------------------

EXTENDS  Sequences,Naturals,TLC,FiniteSets

CONSTANTS any

VARIABLES depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack, _pc

vars == << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack, _pc >>
...
Subscriber == 
              LET Subscriber_start == upperbound(attaque) + 1 IN 
               LET Subscriber_end == upperbound(attaque) + 1 IN
                   Subscriber_start .. Subscriber_end

ProcSet == Broker \cup attaque \cup Subscriber
...
Init == 
    /\ depth = 0
    /\ cp = any
    /\ network = [p \in (Broker \cup Subscriber) |-> <<>>]
    /\ activeClients = {}
    /\ Topics = {"A", "B", "C"}
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, INJ1|->FALSE, INJ2|->FALSE]]
...
Lbl_1(self) == 
               ...
               /\ \/ /\ (({Head(_stack[self]).pkt.sender} \cap activeClients) # {})
                        /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBCOMP", sender |-> self]) IN
                           ...
                           /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
                  \/ /\ ~((({Head(_stack[self]).pkt.sender} \cap activeClients) # {}))
                           /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                           /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
Lbl_2(self) == 
               /\ _pc[self] = "Lbl_2"
               /\ (cp = any \/ cp = self)
               /\ \/ /\ (({Head(_stack[self]).pkt.sender} \cap activeClients) # {})
                                  ...
                                  \/ /\ (pkt.type = "PUBCOMP")
                                        /\ LET _newhead == Head(_stack[self]) IN
                                           ...
                                           \/ /\ (Head(___stack[self]).PUBSUCC = TRUE)
                                                    ...
                                                    /\ _stack' = ___stack
                                                    /\ network' = _network
                                                    /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data >>
                                           \/ /\ ~((Head(___stack[self]).PUBSUCC = TRUE))
                                                    /\ _stack' = ___stack
                                                    /\ network' = _network
            ...
            /\ \/ /\ (Head(_stack[self]).pkt.QoS = 0)
               ...
               \/ /\ (Head(_stack[self]).pkt.QoS = 2)
                     /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBREC", sender |-> self]) IN
                        ...
                        /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                        /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _attaque_data, _Subscriber_data, _stack >>
Lbl_4(self) == 
               /\ _pc[self] = "Lbl_4"
               /\ (cp = any \/ cp = self)
               /\ IF TopicPool[Head(_stack[self]).pkt.topic] # {} /\ Head(_stack[self]).idS_sub = {}
                        THEN /\ LET newHead1 == Head(_stack[self]) IN
                                ...
                                /\ _pc' = [_pc EXCEPT ![self] = "Lbl_4"]
                                /\ _stack' = ___stack
                                /\ UNCHANGED << depth, cp, network, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data >>
                        ELSE
                         ...
                         /\ LET packet == Head(network[self]) IN
                            \/ /\ (packet.type = "CONNECT")
                                  /\ LET _activeClients == (activeClients \cup {packet.sender}) IN
                                     ...
                                     /\ UNCHANGED << depth, cp, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
                            \/ /\ (packet.type = "PUBLISH")
                                  /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                            ...
                            \/ /\ (packet.type = "SUBSCRIBE")
                                  /\ \/ /\ (({packet.sender} \cap activeClients) # {})
                                           /\ LET _TopicPool == [TopicPool EXCEPT ![packet.topic] = (TopicPool[packet.topic] \cup {packet.sender})] IN
                                              ...
                                              /\ network' = __network
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                                              /\ UNCHANGED << depth, cp, activeClients, Topics, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
                                     \/ /\ ~((({packet.sender} \cap activeClients) # {}))
                                           /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                                              /\ network' = _network
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                             ...
                             \/ /\ (packet1.type = "CONNACK")
                                   /\ LET __attaque_data == [_attaque_data EXCEPT ![self].INJ1 = TRUE] IN
                                      ...
                                      \/ /\ (__attaque_data[self].INJ1 = TRUE)
                                               ...
                                               /\ network' = _network
                                               /\ _pc' = [_pc EXCEPT ![self] = "subscriber"]
...
subscriber(self) == 
                    ...
                    /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                             /\ LET packet2 == Head(network[_attaque_data[self].BrokerID]) IN
                                \/ /\ (packet2.type = "UNSUBSCRIBE")
                                      /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "UNSUBSCRIBE", sender |-> packet2.sender, topic |-> "A"]) IN
                                         /\ network' = _network
...
waitsubscription(self) == 
                          ...
                          /\ cp = any
                          /\ \/ /\ (Len(network[3]) > 0)
                                   /\ LET packet3 == Head(network[3]) IN
                                      \/ /\ (packet3.type = "UNSUBACK")
                                      ...
                                      \/ /\ ~((packet3.type = "UNSUBACK"))
                                            /\ LET _network == [network EXCEPT ![3] = Tail(network[3])] IN
                                               ...
                                               \/ /\ ~((_attaque_data[self].INJ2 = TRUE))
                                                        ...
                                                        /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
_attaque(self) == 
                      Lbl_7(self) \/ CONNACK(self) \/ subscriber(self) \/ waitsubscription(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
Sub_start(self) == 
                   /\ _pc[self] = "Sub_start"
                   /\ cp = any
...
Sub(self) == 
             ...
             /\ LET _network == send(_Subscriber_data[self].BrokerID, [type |-> "SUBSCRIBE", topic |-> "A", sender |-> self]) IN
                /\ network' = _network
                /\ _pc' = [_pc EXCEPT ![self] = "Subwaitsubscription"]
                /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
Subwaitsubscription(self) == 
                             /\ _pc[self] = "Subwaitsubscription"
                             /\ cp = any
                             /\ \/ /\ (Len(network[self]) > 0)
                                      /\ LET packet == Head(network[self]) IN
                                         \/ /\ (packet.type = "SUBACK")
...
UNSUB(self) == 
               ...
               /\ LET _network == send(_Subscriber_data[self].BrokerID, [type |-> "UNSUBSCRIBE", topic |-> "A", sender |-> self]) IN
                  /\ network' = _network
                  /\ _pc' = [_pc EXCEPT ![self] = "Subwaitunsubscription"]
                  /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
Subwaitunsubscription(self) == 
                               /\ _pc[self] = "Subwaitunsubscription"
                               /\ cp = any
                               /\ \/ /\ (Len(network[self]) > 0)
                                        /\ LET packet == Head(network[self]) IN
                                           ...
                                           \/ /\ ~((packet.type = "UNSUBACK"))
                                                 /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                                                    ...
                                                    \/ /\ ~((_Subscriber_data[self].UNSUBSUCC = TRUE))
                                                             ...
                                                             /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Subscriber_data, _stack >>
_Subscriber(self) == 
                         Sub_start(self) \/ Sub_waitCONN(self) \/ Sub(self) \/ Subwaitsubscription(self) \/ UNSUB(self) \/ Subwaitunsubscription(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
...
Temp0 == (~ \A D \in Subscriber, I \in attaque : (<> (((_Subscriber_data[D].UNSUBSUCC = TRUE) /\ (_attaque_data[I].INJ1 = TRUE)) => (_attaque_data[I].INJ2 = TRUE))))
