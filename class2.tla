...
Init == 
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, INJ|->FALSE]]
                             ...
                             \/ /\ ~((packet1.type = "CONNACK"))
                                   /\ LET _network == [network EXCEPT ![3] = Tail(network[3])] IN
                                      ...
                                      \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                               ...
                                               /\ UNCHANGED << depth, cp, activeClients, Topics, TopicPool, _Broker_data, _attaque_data, _Publisher_data, _stack >>
_attaque(self) == 
                      Lbl_7(self) \/ CONNACK(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ rqos2(self) \/ waitPUBREL(self)  
PubStart(self) == 
                  /\ _pc[self] = "PubStart"
                  /\ cp = any
...
Inv0 == ((activeClients \cap {2}) = {})
Temp0 == (~ \A D \in Publisher, I \in attaque : (<> ((_Publisher_data[D].CONNSUCC = TRUE) => (_attaque_data[I].INJ = TRUE))))
